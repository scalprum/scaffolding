...
describe('useSubscribeStore', () => {
  ...
  describe('Basic functionality', () => {
    it('should return initial selected value from primitive state', () => {
      ...
      const { result } = renderHook(() => useSubscribeStore(store, 'INCREMENT', (state) => state * 2));
    ...
    it('should return initial selected value from object state', () => {
      ...
      const { result } = renderHook(() => useSubscribeStore(store, 'UPDATE_COUNT', (state) => state.count));
    ...
    it('should return selected object and ensure immutability on updates', () => {
      ...
      const { result } = renderHook(() => useSubscribeStore(store, 'UPDATE_USER', (state) => state.user));
    ...
    it('should return selected array and ensure immutability on updates', () => {
      ...
      const { result } = renderHook(() => useSubscribeStore(store, 'ADD_ITEM', (state) => state.items));
  ...
  describe('Event-specific subscription', () => {
    ...
    it('should update only for specific event even with same selector', () => {
      ...
      const { result: countResult } = renderHook(() => useSubscribeStore(store, 'UPDATE_COUNT', countSelector));
      const { result: nameResult } = renderHook(() => useSubscribeStore(store, 'UPDATE_NAME', nameSelector));
  ...
  describe('Selector function behavior', () => {
    ...
    it('should handle array transformations', () => {
      const { result } = renderHook(() =>
        useSubscribeStore(store, 'UPDATE_ITEMS', (state) => state.items.map((item: { id: number; name: string }) => item.name).join(', ')),
      );
    ...
    it('should handle nested property selection', () => {
      const { result } = renderHook(() => useSubscribeStore(store, 'UPDATE_SETTINGS', (state) => state.settings.theme));
    ...
    it('should handle computed values', () => {
      ...
      const { result } = renderHook(() => useSubscribeStore(store, 'UPDATE_ITEMS', computedSelector));
  ...
  describe('Subscription management', () => {
    it('should unsubscribe when hook unmounts', () => {
      ...
      const { unmount } = renderHook(() => useSubscribeStore(store, 'INCREMENT', (state) => state.count));
    ...
    it('should not update after unmount', () => {
      ...
      const { result, unmount } = renderHook(() => useSubscribeStore(store, 'INCREMENT', (state) => state.count));
    ...
    it('should handle multiple hooks with different events', () => {
      ...
      const { result: countResult } = renderHook(() => useSubscribeStore(store, 'UPDATE_COUNT', (state) => state.count));
      const { result: nameResult } = renderHook(() => useSubscribeStore(store, 'UPDATE_NAME', (state) => state.name));
  ...
  describe('Edge cases and complex scenarios', () => {
    it('should handle selector returning primitives', () => {
      ...
      const { result } = renderHook(() => useSubscribeStore(store, 'UPDATE', (state) => state.count > 10));
    ...
    it('should handle selector returning null', () => {
      ...
      const { result } = renderHook(() => useSubscribeStore(store, 'SET_USER', (state) => state.user));
      ...
      const { result: clearResult } = renderHook(() => useSubscribeStore(store, 'CLEAR_USER', (state) => state.user));
    ...
    it('should handle selector errors gracefully', () => {
      ...
      const { result } = renderHook(() => useSubscribeStore(store, 'UPDATE', riskySelector));
    ...
    it('should maintain referential equality for same computed values', () => {
      ...
      const { result, rerender } = renderHook(() => useSubscribeStore(store, 'UPDATE', (state) => state.items.length));
    ...
    it('should work with different store instances', () => {
      ...
      const { result: result1 } = renderHook(() => useSubscribeStore(store1, 'UPDATE', (state) => state.value));
      const { result: result2 } = renderHook(() => useSubscribeStore(store2, 'UPDATE', (state) => state.value));
