...
import { TypedHookConfig, UseRemoteHookResult, HookHandle, RemoteHookManager } from './remote-hooks-types';
...
export function useRemoteHookManager<R = unknown>(): RemoteHookManager<R> {
  ...
  const addHook = useCallback(
    <S extends string, M extends string, I extends string | undefined = undefined>(config: TypedHookConfig<S, M, I>): HookHandle => {
      ...
      const loadHook = async () => {
        try {
          const hookFunction = await getModule<(...args: any[]) => any>(config.scope, config.module, config.importName);
